...
public class Prospector extends Item {
        ...
        if(!world.isClient) {
            ...
            for (int i = -64; i < blockPos.getY()+64; i++) {
                ...
                if(isRightBlock(state)){
                    ...
                    context.getWorld().playSound(null,blockPos,
                            ModSounds.PROSPECTOR_CALL, SoundCategory.BLOCKS,1f,1f);
                    break;
                }
            }
            if(!foundBlock){
                player.sendMessage(Text.literal("此处无铁矿"));
            }
        }
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        tooltip.add(Text.translatable("item.casualthings.prospector.tooltip"));
